...
import { ERASER_OPACITY, HIGHLIGHTER_OPACITY, PEN_OPACITY, WIDTH_SCALE } from '../constants/tools'
...
export function useDrawing(
  ...
  function setupCanvases(width: number, height: number, scale: number) {
    dpr = Math.min(scale, Math.sqrt(MAX_CANVAS_PIXELS / Math.max(1, width * height)))
    dpr = Math.max(1, dpr)
  ...
  function drawAction(ctx: CanvasRenderingContext2D, action: DrawAction) {
    if (action.tool === 'eraser') {
      ...
      drawSmoothSegment(ctx, action.points, action.color, action.lineWidth, action.opacity)
    }
  }
