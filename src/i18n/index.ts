import { ref } from "vue";

type Messages = Record<string, string>;

const en: Messages = {
  "tool.pen": "Pen",
  "tool.highlighter": "Highlighter",
  "tool.eraser": "Eraser",
  "tool.rect": "Rectangle",
  "tool.line": "Line",
  "tool.circle": "Circle",
  "tool.arrow": "Arrow",
  "tool.text": "Text",
  "tool.fading": "Fading Pen",
  "tool.blur": "Blur",
  "action.undo": "Undo",
  "action.redo": "Redo",
  "action.clear": "Clear",
  "action.exit": "Exit",
  "action.penetrate": "Click through",
  "action.spotlight": "Spotlight",
  "action.zoom": "Zoom (M/Z)",
  "action.boardWhite": "Whiteboard (B)",
  "action.boardBlack": "Blackboard (B)",
  "action.export": "Export PNG",
  "action.copy": "Copy to clipboard (Ctrl+C)",
  "action.copied": "Copied to clipboard",
  "action.copyFailed": "Copy failed",
  "action.customColor": "Custom color",
  "action.recentColor": "Recent color",
  "action.exporting": "Exporting…",
  "action.exported": "Saved",
  "action.exportFailed": "Export failed",
  "action.captureFailed": "Screen capture failed",
  "action.blurReady": "Mosaic base ready — draw now",
  "action.penetrationFailed": "Failed to toggle click-through",
  "action.textPlaceholder": "Type text, Enter to place",
  "action.zoomInBoard": "Screen zoom is unavailable in board mode",
  "action.penetrateInBoard": "Click-through is unavailable in board mode",
  "error.initFailed":
    "AkiMark failed to initialize — annotation may not work. Try restarting the app.",
  "settings.title": "Settings",
  "settings.language": "Language",
  "settings.preserveDrawings": "Keep drawings on exit",
  "settings.preserveDrawingsDesc":
    "Drawings stay when the overlay hides; they reappear on next activation",
  "settings.shortcuts": "Global Shortcuts",
  "settings.toggleDrawing": "Start / stop drawing",
  "settings.clearDrawing": "Clear screen",
  "settings.togglePenetration": "Toggle click-through",
  "settings.recordHint": "Click, then press the key combination",
  "settings.autostart": "Launch at startup",
  "settings.autostartDesc": "Run AkiMark silently in the background on login",
  "settings.defaultTool": "Default tool",
  "settings.defaultColor": "Default color",
  "settings.lineWidths": "Line widths",
  "settings.openSettingsOnStartup": "Open settings on startup",
  "settings.save": "Save",
  "settings.saved": "Saved",
  "settings.shortcutConflict": "Shortcut occupied by another app",
  "settings.shortcut-empty": "Shortcut cannot be empty",
  "settings.shortcut-duplicate": "Shortcuts cannot be the same",
  "settings.shortcutConflictBanner":
    "Some global shortcuts are occupied by other apps and could not be registered:",
  "settings.helpTitle": "Shortcuts & Features",
  "settings.helpGlobal": "Global hotkeys (work anywhere)",
  "settings.helpInApp": "In-overlay shortcuts",
  "settings.helpMouse": "Mouse",
  "settings.helpGlobalToggle": "Start / stop drawing",
  "settings.helpGlobalClear": "Clear screen",
  "settings.helpGlobalPenetrate": "Toggle click-through",
  "settings.helpIn1":
    "1-8: Pen / Fading Pen / Highlighter / Eraser / Line / Rect / Circle / Arrow · 9: Text · 0: Blur",
  "settings.helpInQE": "Cycle color (previous / next)",
  "settings.helpInSpace": "Toggle toolbar",
  "settings.helpInX": "Toggle click-through",
  "settings.helpInF": "Toggle spotlight (scroll adjusts radius)",
  "settings.helpInM":
    "Toggle screen zoom (M / Z: click to focus at cursor, scroll changes 2×–8×)",
  "settings.helpInB":
    "Toggle board mode (B: white/black board — screen zoom & click-through are disabled while on)",
  "settings.helpInS": "Export screenshot (PNG)",
  "settings.helpInCtrlC": "Copy annotation to clipboard",
  "settings.helpInCtrlD": "Clear screen",
  "settings.helpInCtrlZY": "Undo / Redo",
  "settings.helpInEsc":
    "Exit annotation mode (Esc steps out: zoom → spotlight → board → annotation)",
  "settings.helpMouseDraw": "Left-drag to draw",
  "settings.helpMouseErase": "Hold right-button to erase temporarily",
  "settings.helpTip": "Tip: hold right-button to erase without switching tools",
  "settings.helpMore": "Other features",
  "settings.helpTray":
    "Tray resident — left-click tray icon toggles drawing, right-click opens menu",
  "settings.helpAutoPenetrate":
    "Auto click-through after losing focus (won't block other apps)",
  "settings.helpPressure":
    "Pressure: pen tablets get real pressure; mouse/trackpad draw at constant width",
  "settings.helpConflict":
    "Shortcut conflicts with other apps are detected and shown on save",
  "settings.exportDir": "Export folder",
  "settings.exportDirDesc": "Where exported PNG screenshots are saved",
  "settings.exportDirChoose": "Choose folder…",
  "settings.exportDirDefault": "Desktop (default)",
  "settings.boardDefault": "Default board color",
  "settings.boardWhite": "White",
  "settings.boardBlack": "Black",
};

const zhCN: Messages = {
  "tool.pen": "画笔",
  "tool.highlighter": "荧光笔",
  "tool.eraser": "橡皮",
  "tool.rect": "矩形",
  "tool.line": "直线",
  "tool.circle": "圆形",
  "tool.arrow": "箭头",
  "tool.text": "文字",
  "tool.fading": "渐隐笔",
  "tool.blur": "马赛克笔",
  "action.undo": "撤销",
  "action.redo": "重做",
  "action.clear": "清屏",
  "action.exit": "退出",
  "action.penetrate": "点击穿透",
  "action.spotlight": "聚光灯",
  "action.zoom": "屏幕缩放 (M/Z)",
  "action.boardWhite": "白板 (B)",
  "action.boardBlack": "黑板 (B)",
  "action.export": "导出 PNG",
  "action.copy": "复制到剪贴板（Ctrl+C）",
  "action.copied": "已复制到剪贴板",
  "action.copyFailed": "复制失败",
  "action.customColor": "自定义颜色",
  "action.recentColor": "最近使用",
  "action.exporting": "正在导出…",
  "action.exported": "已保存",
  "action.exportFailed": "导出失败",
  "action.captureFailed": "截图失败",
  "action.blurReady": "马赛克底图已就绪，可以绘制",
  "action.penetrationFailed": "切换穿透模式失败",
  "action.textPlaceholder": "输入文字，回车放置",
  "action.zoomInBoard": "板书模式下无法使用屏幕缩放",
  "action.penetrateInBoard": "板书模式下无法使用鼠标穿透",
  "error.initFailed":
    "AkiMark 初始化失败，标注功能可能不可用，请尝试重启应用。",
  "settings.title": "设置",
  "settings.language": "界面语言",
  "settings.preserveDrawings": "退出时保留笔迹",
  "settings.preserveDrawingsDesc": "隐藏标注层时不清空笔迹，下次唤出时恢复显示",
  "settings.shortcuts": "全局快捷键",
  "settings.toggleDrawing": "开始 / 结束标注",
  "settings.clearDrawing": "清屏",
  "settings.togglePenetration": "切换穿透模式",
  "settings.recordHint": "点击后按下新的组合键",
  "settings.autostart": "开机自启动",
  "settings.autostartDesc": "登录后在后台静默运行，几乎不占资源",
  "settings.defaultTool": "默认画笔工具",
  "settings.defaultColor": "默认颜色",
  "settings.lineWidths": "线宽",
  "settings.openSettingsOnStartup": "启动时打开设置",
  "settings.save": "保存",
  "settings.saved": "已保存",
  "settings.shortcutConflict": "快捷键被其他程序占用",
  "settings.shortcut-empty": "快捷键不能为空",
  "settings.shortcut-duplicate": "快捷键不能重复",
  "settings.shortcutConflictBanner": "以下全局快捷键被其他程序占用，未能注册：",
  "settings.helpTitle": "快捷键与功能介绍",
  "settings.helpGlobal": "全局快捷键（任何界面可用）",
  "settings.helpInApp": "标注界面内快捷键",
  "settings.helpMouse": "鼠标操作",
  "settings.helpGlobalToggle": "开始 / 结束标注",
  "settings.helpGlobalClear": "清屏",
  "settings.helpGlobalPenetrate": "切换穿透模式",
  "settings.helpIn1":
    "1-8：画笔 / 渐隐笔 / 荧光笔 / 橡皮 / 直线 / 矩形 / 圆形 / 箭头 · 9：文字 · 0：马赛克笔",
  "settings.helpInQE": "循环切换颜色（上一个 / 下一个）",
  "settings.helpInSpace": "显示 / 隐藏工具栏",
  "settings.helpInX": "切换穿透模式",
  "settings.helpInF": "切换聚光灯（滚轮调节半径）",
  "settings.helpInM": "切换屏幕缩放（M / Z：点击以光标处聚焦，滚轮切换 2×–8×）",
  "settings.helpInB":
    "开关板书模式（B：白板 / 黑板 —— 板书期间屏幕缩放与鼠标穿透不可用）",
  "settings.helpInS": "导出截图（PNG）",
  "settings.helpInCtrlC": "复制标注到剪贴板",
  "settings.helpInCtrlD": "清屏",
  "settings.helpInCtrlZY": "撤销 / 重做",
  "settings.helpInEsc":
    "退出标注模式（Esc 逐级退出：缩放 → 聚光灯 → 板书 → 标注）",
  "settings.helpMouseDraw": "左键拖拽绘制",
  "settings.helpMouseErase": "按住右键临时擦除",
  "settings.helpTip": "提示：无需切换工具，按住右键即可临时擦除",
  "settings.helpMore": "其他特性",
  "settings.helpTray": "托盘常驻 — 左键单击托盘图标切换标注，右键打开菜单",
  "settings.helpAutoPenetrate": "失焦后自动穿透（不会挡住其他应用操作）",
  "settings.helpPressure": "压感：数位笔真实压感；鼠标/触控板以恒定宽度绘制",
  "settings.helpConflict": "与其他程序冲突的快捷键会被检测并在保存时提示",
  "settings.exportDir": "导出目录",
  "settings.exportDirDesc": "导出截图（PNG）的保存位置",
  "settings.exportDirChoose": "选择文件夹…",
  "settings.exportDirDefault": "桌面（默认）",
  "settings.boardDefault": "默认板书底色",
  "settings.boardWhite": "白板",
  "settings.boardBlack": "黑板",
};

const catalogs: Record<string, Messages> = {
  en,
  "zh-CN": zhCN,
};

const locale = ref<"en" | "zh-CN">(
  (navigator.language.startsWith("zh") ? "zh-CN" : "en") as "en" | "zh-CN",
);

/** 极简 i18n：dot-path 查表，无外部依赖 */
export function useI18n() {
  const t = (key: string): string => catalogs[locale.value][key] ?? key;

  return {
    locale,
    t,
    setLocale: (l: "en" | "zh-CN") => {
      locale.value = l;
    },
  };
}
